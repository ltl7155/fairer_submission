...
class PruningConfigReader(object):
    ...
    def read_config(self, filename):
        ...
        print (config)
        self.pruning_settings["pruning_method"]= self.pruning_settings["method"]
...
class pytorch_pruning(object):
    def __init__(self, parameters, pruning_settings=dict(), log_folder=None,writer=None):
        ...
        initialize_parameter(self, pruning_settings, 'pruning_method', 0)
    ...
    def add_criteria(self):
        ...
        for layer, if_prune in enumerate(self.prune_layers):
            ...
            if len(self.pruning_mask_from) > 0:
                ...
                self.method = -1
                criteria_for_layer = torch.from_numpy(self.loaded_mask_criteria[layer]).type(torch.FloatTensor).to(device)
    ...
    def compute_saliency(self):
        ...
        print("prune_neurons_now",prune_neurons_now,
               "len(all_criteria)-1", len(all_criteria)-1,
                "prune_neurons_now",prune_neurons_now, 
                "self.prune_neurons_max//self.group_size - 1 ", 
                self.prune_neurons_max//self.group_size - 1 )
            ...
            for group in groups[layer]:
                if group[1] <= threshold_now:
                    for unit in group[0]:
                        ...
                        # **************
                        # **************
                        if np.nonzero(self.pruning_gates[layer])[0].size > 5:
                            self.pruning_gates[layer][unit] *= 0.0
                            self.parameters[layer].data[unit] *= 0.0
                            
            print("pruned_perc:", [np.nonzero(1.0-self.pruning_gates[layer])[0].size, len(self.parameters[layer])])
        ...
        print ('all_neuron_units:prune',all_neuron_units, self.pruned_neurons)
    ...
    def set_weights_oracle_pruning(self):
        ...
        for layer,if_prune in enumerate(self.prune_layers_oracle):
            ...
            if self.method == 40:
                self.parameters[layer].data = deepcopy(torch.from_numpy(self.stored_weights).to(device))
...
#                     print("layer:", layer, "unit", unit)
    ...
    def compute_hessian(self, loss):
        ...
        for layer_indx, parameter in enumerate(self.parameters):
            ...
            self.temp_hessian.append(torch.FloatTensor(hessian.cpu().numpy()).to(device))
    ...
    def report_loss_neuron(self, training_loss, training_acc, train_writer = None, neurons_left = 0):
        ...
        if train_writer is not None:
            train_writer.add_scalar('prune/loss_neuron', training_loss, self.all_neuron_units-self.neuron_units)
    ...
    def report_to_tensorboard(self, train_writer=None, processed_batches=None):
        '''
        ...
        # train_writer.add_scalar('prunex/Neurons_left', self.neuron_units, processed_batches)
        train_writer.add_scalar('prune/Neurons_left', self.neuron_units, self.pruning_iterations_done)
        train_writer.add_scalar('prune/Criteria_min', self.min_criteria_value, self.pruning_iterations_done)
        train_writer.add_scalar('prune/Criteria_max', self.max_criteria_value, self.pruning_iterations_done)
        train_writer.add_scalar('prune/Criteria_median', self.median_criteria_value, self.pruning_iterations_done)
        train_writer.add_scalar('prune/Gradient_norm_before', gradient_norm_final_before, self.pruning_iterations_done)
        if hasattr(self,"threshold_now"):
            train_writer.add_scalar('prune/Pruning_threshold', self.threshold_now, self.pruning_iterations_done)
    ...
    def do_step(self, loss=None, optimizer=None, neurons_left=0, training_acc=0.0, pretest=None, test=False):
        '''
        ...
        # sets pruned weights to zero
#         print("retrain_para:", self.parameters[17].data[283])
#         print("retrain_para2:", self.parameters[17].data[280])
        self.enforce_pruning()
#         print("para:", self.parameters[17].data[283])
#         print("para2:", self.parameters[17].data[280])
        
#         if test:
#             pretest(namespace="MidTest0")
