...
def evaluate_mask(model, dataloader, mask):
    y_scores = []
    y_scores_masked = []
    y_true = []
    for i, (inputs, target) in enumerate(dataloader):
...
def evaluate_raw(model, dataloader):
    y_scores = []
    y_true = []
    for i, (inputs, target) in enumerate(dataloader):
        inputs, target = inputs.to(device), target.float().to(device)
...
def evaluate_dp(model, dataloader, dataloader_0, dataloader_1):
    
    model.eval()

    ap, _, _ = evaluate_raw(model, dataloader)
    # calculate DP gap
    _, pred_0, y_test_0 = evaluate_raw(model, dataloader_0)
    _, pred_1, y_test_1 = evaluate_raw(model, dataloader_1)
...
def evaluate_eo(model,  dataloader, dataloader_00, dataloader_01, dataloader_10, dataloader_11):
    model.eval()

    # calculate average precision
    ap, _, _ = evaluate_raw(model, dataloader)

    _, pred_00, y_test_00 = evaluate_raw(model, dataloader_00)
    _, pred_01, y_test_01 = evaluate_raw(model, dataloader_01)
    _, pred_10, y_test_10 = evaluate_raw(model, dataloader_10)
    _, pred_11, y_test_11 = evaluate_raw(model, dataloader_11)
