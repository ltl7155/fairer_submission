...
def train_van(epochs, model, dataloader, pretest_call):
    model.train()
    ...
    for it in tqdm(range(niter)):
        ...
        loss.backward()
        optimizer.step()
        writer.add_scalar('Train/loss', loss, niter * epochs + it)
        if it == niter - 1:
...
#     output_sizes = get_conv_sizes(args, model)
    ...
    log_folder = f"logs_alexnet/{args.fair_method}/Exp{i}_{args.mode}_{args.name}_{args.fair_method}"
#     pth_folder = f"{args.save_root}/save_alexnet/Exp{i}_{args.mode}_{args.name}_{args.fair_method}"
...
#     os.makedirs(pth_folder, exist_ok=True)
    ...
    for j in range(0, args.epochs):
        ...
        if args.mode == "dp":
            ...
            train_dataloader = get_loader(train_df, os.path.join(data_celeba_dir, 'celeba/split/train/'), target_id, args.bs*2)
            valid_dataloader = get_loader(valid_df, os.path.join(data_celeba_dir, 'celeba/split/val/'), target_id, args.bs*2)
            test_dataloader = get_loader(test_df, os.path.join(data_celeba_dir, 'celeba/split/test/'), target_id, args.bs*2)
            ...
            metainfo = train_van(j, model,  train_dataloader, pretest_call)

        elif args.mode == "eo":
            
            train_dataloader = get_loader(train_df, os.path.join(data_celeba_dir, 'celeba/split/train/'), target_id, args.bs*4)
            valid_dataloader = get_loader(valid_df, os.path.join(data_celeba_dir, 'celeba/split/val/'), target_id, args.bs*4)
            test_dataloader = get_loader(test_df, os.path.join(data_celeba_dir, 'celeba/split/test/'), target_id, args.bs*4)
...
#     os.makedirs(f"results_alexnet_van/Normal/{args.fair_method}", exist_ok=True)
#     fname = f"results_alexnet_van/Normal/{args.fair_method}/Exp{i}_{args.mode}_{args.name}_{args.fair_method}.txt"
